"""
Backfill chunk text in the SQLite database.

One-time script to re-read source files and populate chunk_text
for all existing DB rows that have empty text.
"""
...
def main():
    ...
    # Fetch all empty chunks
    cur = conn.execute(
        """SELECT chunk_id, source_path, start_offset, end_offset 
           FROM chunks 
    ...
    for chunk_id, source_path, start_offset, end_offset in rows:
        ...
        if start_offset < len(content) and end_offset <= len(content):
            ...
            # End offset exceeds file, take what we can
